...
{
  public SegregationDirectory GetFiles(DirectoryInfo directoryInfo, string searchPattern, CancellationToken cancellationToken)
  {
    cancellationToken.ThrowIfCancellationRequested();
    var segregationDirectory = new SegregationDirectory { DirectoryName = directoryInfo.FullName };
    try
    {
      ...
      segregationDirectory.Files = [.. files.Select(
        x => new SegregationFile { FileName = x.Name, Status = Status.Unprocessed })];
    }
    catch (OperationCanceledException operationCanceledException)
    {
